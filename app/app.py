...
# BASE_URL = "http://localhost:8000/api"  # Adjust this to match your FastAPI server URL
#
# AUTH = "auth"
# USERS = "users"
#
...
#         else:
#             st.error(response.json().get("detail", "Failed to get user details"))
#
# # Main app function
# def main():
#     st.title("FastAPI with JWT Authentication")
#
#     menu = ["Register", "Login", "Refresh Token", "Logout", "Get User Info", "Delete All Users", "Get All Users", "Get User Details"]
...
# Get all users function
def get_all_users():
    st.subheader("Get All Users (ID, Email, Name)")
    ...
    if response.status_code == 200:
        ...
        if all("id" in user for user in users):  # Uncomment if needed
            ...
            selected_users = [
                {"ID": user["id"], "Email": user["email"], "Name": user.get("name", ""), "Created at": user["created_at"]}
                for user in users
            ]
